...
export function presentationObjectTools(selection: PresentationElement[]) {
  ...
  const isLine = (element: PresentationElement) => element.type === "shape" && (Boolean(element.content.connection) || element.content.shape === "line" || element.content.shape === "arrow" || element.content.shape === "doubleArrow");
  return {
