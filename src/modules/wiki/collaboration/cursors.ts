          ...
          decorations(state) {
            ...
            for (const person of provider.people) {
              ...
              try {
                ...
                if (anchor !== head) decorations.push(Decoration.inline(Math.min(anchor, head), Math.max(anchor, head), { style: `background:${userIdentityColor(person.userId, "highlight")}` }));
                decorations.push(Decoration.widget(head, () => {
                  const cursor = document.createElement("span");
                  cursor.style.cssText = `border-left:2px solid ${userIdentityColor(person.userId)};position:relative;pointer-events:none`;
                  const label = document.createElement("span"); label.textContent = person.name;
                  label.style.cssText = `position:absolute;bottom:100%;left:0;background:${userIdentityColor(person.userId)};color:white;font:10px sans-serif;padding:2px 4px;white-space:nowrap;border-radius:3px`;
