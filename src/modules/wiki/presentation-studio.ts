...
export async function getPresentationStudio(id: string) {
  ...
  const library = db.select().from(wikiPresentationLibrary).orderBy(desc(wikiPresentationLibrary.createdAt)).limit(100).all().map((entry) => ({ id: entry.id, name: entry.name, createdBy: entry.createdBy, kind: entry.kind, removable: entry.createdBy === viewer.id || viewer.role === "admin", theme: entry.kind === "theme" ? presentationThemeSchema.parse(JSON.parse(entry.documentJson)) : undefined }));
  const assets = db.select().from(attachments).orderBy(desc(attachments.createdAt)).all().filter((entry) => entry.mimeType.startsWith("image/") && canUseAttachment(entry, viewer)).slice(0, 200).map((entry) => ({ id: entry.id, name: entry.fileName, uploadedBy: entry.uploadedBy }));
  return { role, access: { restricted: access?.restricted ?? false, coediting: access?.coediting ?? false, publicEnabled: Boolean(access?.publicTokenHash) }, members, users, comments: comments.map(({ authorId, ...comment }) => ({ ...comment, authorId, canManage: role !== "view" && authorId === viewer.id })), library, assets };
}
