          ...
          <div className="rounded-2xl border border-[#dce5e1] dark:border-border bg-white dark:bg-card p-5">
            ...
            <div className="mt-5 space-y-3">
              {data.people.map((person) => <article key={person.id} className="grid items-center gap-4 rounded-xl border border-[#e2e9e6] dark:border-border px-4 py-3 md:grid-cols-[1fr_auto_auto_auto]">
                <div><p className="font-medium text-[#203f36] dark:text-foreground">{person.name}</p><p className="text-xs text-[#71807a] dark:text-muted-foreground">{person.locationName} · {employmentLabel(person.employmentType as PayrollEmploymentType, locale)}</p></div>
...
function PeoplePanel({ data, t, locale, pending, run }: { data: PersonnelWorkspaceData; t: Text; locale: string; pending: boolean; run: (action: () => Promise<unknown>, success: string) => void }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
    ...
    <section className="rounded-2xl border border-[#dce5e1] dark:border-border bg-white dark:bg-card p-5">
      ...
      <div className="mt-4 divide-y divide-[#e7ecea]">{data.people.map((person) => <div key={person.id} className="grid gap-3 py-3 sm:grid-cols-[1fr_auto_auto]"><div><p className="font-medium text-[#173c32] dark:text-foreground">{person.name}</p><p className="text-xs text-[#71807a] dark:text-muted-foreground">{person.personnelNumber || "—"} · {person.collectiveAgreement || t.noCollectiveAgreement}</p></div><div className="text-right"><p className="text-xs text-[#71807a] dark:text-muted-foreground">{t.annual}</p><p className="font-medium tabular-nums">{person.annual ? euro(person.annual.annualEmployerCostCents, locale) : "—"}</p></div><span className={`h-fit rounded-full px-2 py-1 text-xs ${person.annual ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}>{person.annual ? t.complete : t.incomplete}</span></div>)}</div>
    </section>
  </div>;
}
...
function ProjectsPanel({ data, t, locale, pending, run }: PanelProps) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[0.65fr_1.35fr]"><section className="rounded-2xl border border-[#dce5e1] dark:border-border bg-white dark:bg-card p-5"><SectionTitle eyebrow={t.projects} title={t.allocate} /><form className="mt-5 space-y-3" onSubmit={(event) => {
...
function AllocationTable({ data, locale, t }: { data: PersonnelWorkspaceData; locale: string; t: Text }) {
  ...
  return <section className="rounded-2xl border border-[#dce5e1] dark:border-border bg-white dark:bg-card p-5"><SectionTitle eyebrow={t.capacity} title={t.projects} /><div className="mt-4 space-y-4">{[...groups.values()].map((rows) => {
    ...
    return <article key={`${rows[0].employeeId}:${rows[0].payrollMonth}`} className={`rounded-xl border p-4 ${allocation.overbooked ? "border-amber-300 bg-amber-50/40" : "border-[#e2e9e6] dark:border-border"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium text-[#173c32] dark:text-foreground">{rows[0].employeeName} · {rows[0].payrollMonth}</p><p className="text-xs text-[#71807a] dark:text-muted-foreground">{t.remaining}: {number(allocation.remainingMinutes / 60, locale)} h</p></div><span className={`rounded-full px-2 py-1 text-xs font-medium ${allocation.overbooked ? "bg-amber-100 text-amber-900" : "bg-blue-50 text-[#315c7b] dark:text-foreground"}`}>{allocation.overbooked ? t.overbooked : t.capacity}</span></div><div className="mt-3 space-y-2">{rows.map((row) => <div key={row.id} className="grid grid-cols-[1fr_auto_auto] gap-4 text-sm"><span>{row.projectName}</span><span className="tabular-nums">{number(row.plannedMinutes / 60, locale)} h</span><strong className="tabular-nums">{euro(Math.round(row.plannedMinutes * row.costRateCents / 60), locale)}</strong></div>)}</div></article>;
