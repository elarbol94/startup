...
import { UserIdentity, UserIdentityStack } from "@/components/user-identity";
  ...
  return (
    <Select multiple value={value} onValueChange={onChange}>
      <SelectTrigger id={id} className="w-full" aria-label={t("assignees")}>
        <SelectValue>{value.length ? <UserIdentityStack people={value.map(id => ({ id, name: options.find(member => member.id === id)?.name ?? id }))} /> : label}</SelectValue>
      </SelectTrigger>
      <SelectContent>
