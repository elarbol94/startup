...
const icons = { openTasks: ClipboardCheck, upcomingDeadlines: CalendarClock, overdue: AlertTriangle, eventsToday: CalendarDays, documentsCount: FileText, presentationsCount: Presentation, projectsCount: FolderKanban, unreadNews: Bell };
export function OverviewMetric({ id, summary, counts, records }: { id: CardId; summary: ReturnType<typeof getPersonalWorkSummary>; counts: OverviewCounts; records: OverviewRecord[] }) {
  ...
  return <section className="h-full min-w-0 bg-card"><Dialog><DialogTrigger disabled={editing} className="flex h-full w-full min-w-0 items-center gap-2.5 p-3 text-left transition-colors enabled:cursor-pointer enabled:hover:bg-muted/40 focus-visible:outline-2 focus-visible:outline-ring sm:gap-3 sm:p-4" aria-label={t("showMetricDetails", { metric: t(id) })}>
    <span className={`grid size-8 shrink-0 place-items-center rounded-lg sm:size-9 sm:rounded-xl ${id === "overdue" && values.overdue ? "bg-red-50 text-red-600 dark:bg-red-950/40 dark:text-red-300" : "bg-muted text-muted-foreground"}`}><Icon className="size-4" /></span>
    <div className="min-w-0 flex-1">
      <p className="text-2xl leading-tight font-semibold tabular-nums tracking-tight sm:text-3xl">{values[id]}</p>
      <p className="text-xs leading-snug text-muted-foreground">{t(id)}</p>
      {id === "upcomingDeadlines" && (next
        ? <p className="mt-1 text-xs" title={`${dashboard("nextDeadline")}: ${next.title}`}><span className="block truncate font-medium">{next.title}</span><span className="block truncate text-muted-foreground">{dashboard("nextDeadline")} · {format.dateTime(new Date(`${next.deadlineDate}T12:00:00`), { dateStyle: "medium" })}</span></p>
        : <p className="mt-1 text-xs text-muted-foreground">{dashboard("noNextDeadline")}</p>)}
    </div>
